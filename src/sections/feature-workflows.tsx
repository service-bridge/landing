...
const NODE_COLOR: Record<NodeType, string> = { rpc: "text-blue-300", event: "text-emerald-300", sleep: "text-violet-300" };
const NODE_BG: Record<NodeType, string> = { rpc: "bg-blue-500/[0.08]", event: "bg-emerald-500/[0.08]", sleep: "bg-violet-500/[0.08]" };
...
const WAVES: WaveRow[] = [
  {
    tag: null,
    tagColor: "", tagBg: "", tagBorder: "",
    nodes: [{ label: "merchant.validate", type: "rpc", status: "success" }],
    note: null,
  },
  {
    tag: "∥ parallel",
    tagColor: "text-fuchsia-300", tagBg: "bg-fuchsia-500/[0.08]", tagBorder: "border-fuchsia-500/20",
    nodes: [
  ...
  {
    tag: "fan-in",
    tagColor: "text-blue-300", tagBg: "bg-blue-500/[0.08]", tagBorder: "border-blue-500/20",
    nodes: [{ label: "merchant.create", type: "rpc", status: "running" }],
    note: "waits for: kyc + billing",
  },
  {
    tag: "◇ if",
    tagColor: "text-amber-300", tagBg: "bg-amber-500/[0.08]", tagBorder: "border-amber-500/20",
    nodes: [{ label: "email.welcome", type: "event", status: "pending", condition: "status=active" }],
    note: null,
  },
  {
    tag: "⏸ sleep",
    tagColor: "text-violet-300", tagBg: "bg-violet-500/[0.08]", tagBorder: "border-violet-500/20",
...
export function WorkflowsSection() {
  ...
  return (
    <FeatureSection
        ...
        <motion.div variants={fadeInUp}>
          <CodePanel title="workflow run · merchant.onboarding">
            <div className="absolute top-2.5 right-4">
              <Badge tone="border-fuchsia-500/20 bg-fuchsia-500/[0.08] text-fuchsia-300">running</Badge>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-col items-center shrink-0" style={{ width: 72 }}>
                      {wave.tag ? (
                        <div className={cn("rounded-lg border px-2 py-1 text-center w-full", wave.tagBorder, wave.tagBg)}>
                          <p className={cn("text-3xs font-mono font-semibold", wave.tagColor)}>{wave.tag}</p>
                        </div>
                      ) : (
                    ...
                    <div className={cn("flex-1 min-w-0 pb-4", isLast && "pb-0")}>
                      <div className={cn("flex gap-2", wave.nodes.length > 1 ? "flex-row" : "flex-col")}>
                        {wave.nodes.map((node) => {
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-2">
                                ...
                                <p className="text-xs font-semibold font-display truncate flex-1">{node.label}</p>
                                <span className={cn("w-1.5 h-1.5 rounded-full shrink-0", STATUS_DOT[node.status])} />
                              </div>
                              <div className="mt-1.5 flex items-center gap-1.5 flex-wrap">
                                <Badge tone={cn(NODE_BG[node.type], NODE_COLOR[node.type])}>{node.type}</Badge>
                                {node.condition && (
      ...
      cards={
        <>
          <FeatureCard variant="compact" icon={GitMerge} title="Fan-out / Fan-in" description="Steps with independent deps start concurrently. A fan-in step waits for all branches before executing." iconClassName="text-fuchsia-400" />
          <FeatureCard variant="compact" icon={GitBranch} title="Conditional branching" description="The if field uses filter expression syntax. False condition marks the step skipped and cascades to all dependents." iconClassName="text-amber-400" />
          <FeatureCard variant="compact" icon={Clock} title="Sleep & event_wait" description="Suspend a run for milliseconds to days with sleep. event_wait suspends until a matching event arrives — no thread held." iconClassName="text-violet-400" />
          <FeatureCard variant="compact" icon={RefreshCcw} title="Exact-step retry" description="Failed step retries from its checkpoint. Successful parallel branches are never re-executed." iconClassName="text-orange-400" />
        </>
      }
    />
  );
