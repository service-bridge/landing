...
const GRPC_SERVICES = [
  {
    name: "Control",
    role: "Серверный стрим сессии: Welcome при старте, Drain при остановке. Cert refresh — отдельным unary RPC.",
  ...
  {
    name: "Registry",
    role: "RegisterAndWatch: SDK регистрирует свои хендлеры и стримит дельта-апдейты графа сервисов и эндпоинтов.",
  ...
  {
    name: "Call · Invoke",
    role: "RPC между сервисами: Call — прямой mTLS caller → callee; Invoke — proxy через рантайм, когда эндпоинт неизвестен.",
  ...
  },
  {
    ...
    role: "Durable-доставка через рантайм: at-least-once, retries, DLQ, idempotency; DAG-шаги воркфлоу и cron/delayed джобы.",
    color: "text-sky-400",
    bg: "bg-sky-500/[0.06]",
...
const LEGEND = [
  { label: "Direct RPC path", value: "0 hops", color: "text-yellow-400" },
  { label: "Event delivery", value: "at-least-once", color: "text-sky-400" },
  { label: "Source of truth", value: "PostgreSQL 18+", color: "text-lime-400" },
  { label: "Control plane", value: "gRPC :14445", color: "text-cyan-400" },
  { label: "Scale", value: "up to 1000 services", color: "text-emerald-400" },
];
  ...
  return (
    <Section id="architecture">
      <SectionHeader
        ...
        subtitle="RPC flows directly between services over mTLS with circuit-breaker protection. Events, jobs, workflows, and traces go through one runtime over the gRPC control plane, with PostgreSQL as the single source of truth."
      />
        ...
        <div className="rounded-2xl border border-surface-border bg-surface p-6 sm:p-8 md:p-10">
          ...
          <div className="mb-5 rounded-xl border border-violet-500/20 bg-violet-500/[0.04] px-4 py-3 flex items-center gap-3">
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-2xs font-mono text-violet-300/80">
              ...
              <span>
                <span className="text-violet-400 font-semibold">mTLS</span> — SPIFFE cert
                identity cryptographically backs every caller
              </span>
            </div>
          </div>
