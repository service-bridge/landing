      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border bg-muted/50 text-left text-3xs uppercase tracking-wider text-muted-foreground">
            <th className="px-4 py-2.5 font-semibold">{locale === "ru" ? "Параметр" : "Parameter"}</th>
            <th className="px-4 py-2.5 font-semibold">{locale === "ru" ? "Тип" : "Type"}</th>
            <th className="px-4 py-2.5 font-semibold">{locale === "ru" ? "По умолч." : "Default"}</th>
            <th className="px-4 py-2.5 font-semibold">{locale === "ru" ? "Описание" : "Description"}</th>
          </tr>
        </thead>
